...
def generate_structure_listing(files):
    ...
    for filepath in files:
        ...
        listing.append(rel_path.replace("\\", "/"))  # Windows kompatibilitás
    return listing

def combine_files(files, output_file):
    with open(output_file, 'w', encoding='utf-8') as outfile:
        # ===== Fájlstruktúra fejlécként =====
        outfile.write("// ====== FÁJLSTRUKTÚRA ======\n")
        structure_listing = generate_structure_listing(files)
        for path in structure_listing:
